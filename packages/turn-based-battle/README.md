...
Use `createSeededRng`, `createManualClock`, and `createSessionRecorder` from `@phaser-game-engines/core` for deterministic sessions. Passing the recorder to a battle records accepted commands; `replaySession` can submit those commands to a controller created with the same seed.
