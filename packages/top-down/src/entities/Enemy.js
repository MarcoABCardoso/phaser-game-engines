...
export default class Enemy extends Entity {
  get attackable() { return this.health > 0; }
  spawn(scene) {
    ...
    scene.physics.add.collider(this.sprite, scene.solids);
  }
  update(scene) {
    ...
    if (boxesOverlap({ x: this.sprite.x, y: this.sprite.y, w: this.sprite.displayWidth, h: this.sprite.displayHeight }, { x: scene.player.x, y: scene.player.y, w: scene.player.displayWidth, h: scene.player.displayHeight })) scene.damagePlayer(this.spec.damage ?? 1, this.sprite);
  }
  ...
  onHit(scene, damage) { if ((this.health -= damage) <= 0) { this.sprite.destroy(); this.sprite = null; scene.onEnemyDefeated(this); } }
